# 工作原理

[English](how-it-works.md) | 简体中文

## 修正优先级

Typo 按以下顺序修正命令：

1. shell 集成提供的别名上下文展开
2. 错误解析
3. 用户规则
4. 历史记录
5. 内置规则
6. 子命令修正
7. 编辑距离匹配
8. 环境变量修正
9. 工具参数修正

这意味着真实报错信息和用户显式覆盖会先于模糊匹配生效。

## Shell 修正上下文

shell 集成可以把当前会话中的 alias、简单包装函数和环境变量名通过
`typo fix --alias-context <file>` 传给 Typo。Typo 会先展开这份上下文，再进入正常
修正链路；当安全时，修正结果会回写为用户原本输入的 alias 形态：

```shell
alias k=kubectl
k lgo

# 按两次 Esc 后
k logs
```

这份上下文是临时数据，不会写入 `~/.typo/`。当前支持 zsh 和 bash alias、fish
abbreviation、PowerShell alias，以及单条命令转发的简单函数包装器。Typo 不会执行任意
函数体；包含管道、重定向、条件分支或多条命令的复杂函数会被忽略。
zsh 集成只会为当前命令写出相关的 alias 或简单函数，避免在插件很多的终端里为了
一次修正而全量扫描整个会话上下文。

同一个上下文文件还会附带当前会话可见的环境变量名。Typo 会把 `$VAR` 和简单 `${VAR}`
展开当作独立修正范围，只和环境变量候选池匹配，例如：

```shell
cd $HOEM/project

# 按两次 Esc 后
cd $HOME/project
```

为了降低 `$PS1` 这类短变量名的误修，环境变量修正使用了比默认命令匹配更严格的相似度阈值。

## 错误解析

当存在真实 `stderr` 输出时，Typo 会优先从报错信息中提取建议。

当前已有文档覆盖的解析器包括：

- `git`：`did you mean...`、明确的 push upstream 建议、分叉 pull rebase 等建议
- `docker`：未知命令建议
- `npm`：命令未找到建议

解析器会保留完整的连字符子命令，例如 `cherry-pick`、`checkout-index` 和 `run-script`。
修正只替换目标词，保留其他参数的引号、空白、管道和重定向。
仅修正命令名的内部接口在处理未闭合引号时，也保留参数中的连续空格、制表符和换行。

通用解析器还支持其他工具的 `did you mean` 提示。
当报错明确给出错误词时，该词必须在参数中唯一出现，且不能紧跟可能消费下一个参数的未知选项。
`--directory=project` 这类明确带有内联值的长选项不会消费下一个参数，因此不妨碍后续错误词的修正。
静态引号中的词也参与唯一性检查，例如目录名 `'addd'` 与子命令 `addd` 会被视为重复。
错误词按完整内容匹配，点号、路径分隔符和引号内空格不会使其被截断；通用解析器不会将 `--` 后的数据参数作为目标。
没有错误词时，只尝试修正紧跟命令名的首个普通参数。
例如 `poetry --directory project addd requests` 需要提示明确指出 `addd`，否则通用解析器放弃该提示。
这可能减少部分工具的提示修正，但可以避免改写目录名等选项值；引擎仍可尝试已有规则和命令树。

包装命令支持 `command --`，以及 `env`、`sudo` 的环境变量赋值和组合短选项。
例如 `sudo -nu root gti status` 只修正 `gti`，不会将用户名当成可执行文件。
`env MODE="hello world" gti status` 和 `env -- MODE=prod gti status` 也会保留赋值并修正后面的命令。
识别赋值和内联选项时只分析语法，不执行其中的变量展开或命令替换。
仅将能保持参数数量的静态或带引号标量值视为赋值；未加引号的动态值、数组及无法确定的间接展开不会用于推断后面的可执行命令。

zsh、bash 和 PowerShell 集成可以自动通过 `typo fix -s <file>` 传入这份 `stderr` 缓存。
在 PowerShell 中，当前首个受支持版本对 native command 的这条链路最稳定。
fish 集成支持当前缓冲区修正，以及空缓冲区时结合上一条历史命令和退出码进行修正；首个受支持版本不会创建或传入 `typo-stderr-*` 实时 `stderr` 缓存。

示例：

```bash
typo fix -s git.stderr "git remove -v"
typo fix -s git.stderr "git push"
typo fix -s docker.stderr "docker psa"
typo fix -s npm.stderr "npm isntall react"
```

## 子命令修正

Typo 不仅能修正顶层命令，也能修正常见工具的子命令。

常见支持工具包括：

- `git`、`docker`、`npm`、`yarn`、`kubectl`、`cargo`、`go`
- `pip`、`brew`、`terraform`、`helm`
- `aws`、`sam`、`cdk`、`eksctl`、`gcloud`、`gsutil`、`az`、`func`、`azd`、`doctl`、`oci`、`linode-cli` 等云平台 CLI

补充说明：

- 常用云工具会作为内置命令候选加入，即使 PATH 发现还没执行，也能先修正顶层命令。
- `git`、`docker`、`kubectl` 内置了树形子命令结构，即使动态发现还没执行，也能修正常见多层子命令。
- 动态发现到的子命令会以 `schema_version: 3` 的树形结构缓存到 `~/.typo/subcommands.json`。
- `git`、`docker`、`aws`、`gcloud`、`az` 支持层级化子命令发现；`kubectl` 资源修正使用保守的内置资源树。
- 不符合 `schema_version: 3` 的旧子命令缓存会被自动隔离，并在后续使用时重新生成。

### 子命令缓存 v3

`subcommands.json` 使用带版本号的树形格式：

```json
{
  "schema_version": 3,
  "tools": [
    {
      "tool": "git",
      "tree": {
        "long_options": ["--help", "--version"],
        "long_options_with_values": ["--git-dir", "--work-tree"],
        "children": {
          "stash": {
            "children": {
              "save": {"terminal": true}
            }
          }
        }
      },
      "updated_at": "2026-04-15T12:00:00Z"
    }
  ]
}
```

v3 保留嵌套命令树格式，并在树节点上增加长选项元数据。这样 Typo 既可以逐层修正命令，例如把 `gcloud container clusers lisr` 修正为 `gcloud container clusters list`，也可以把实验性 `--long-option` 修正所需的候选保存在同一份缓存里。

节点字段含义如下：

- `children`：当前层级下的有效子命令。
- `terminal`：该 token 可以作为命令路径的结束。
- `passthrough`：该 token 后面的内容按用户参数处理，不再继续当作子命令修正。
- `alias`：短别名对应的规范输出名称。
- `long_options`：当前节点可见的已知 `--long-option` token。
- `long_options_with_values`：当前节点可见、且会消费下一个参数的已知 `--long-option` token。

如果 Typo 发现旧格式缓存，包括 v2 或没有 schema 版本的文件，会把该文件重命名为 `subcommands.json.corrupt-<时间戳>`，并在下次使用时重新生成 v3 缓存。该文件只包含动态发现到的命令元数据和选项元数据，不包含用户规则、历史记录或配置，因此隔离旧缓存不会丢失用户数据。

## 本地文件

Typo 会把本地状态保存在 `~/.typo/`：

```text
~/.typo/
├── config.json
├── rules.json
├── usage_history.json
└── subcommands.json
```

各文件作用：

- `config.json`：由 `typo config` 管理的运行配置
- `rules.json`：learn 结果和用户自定义规则
- `usage_history.json`：已接受修正的历史记录；当重复 pair 被提升为用户规则后，条目会带上 `rule_applied: true`
- `subcommands.json`：动态发现到的子命令树缓存

## 配置模型

当前默认暴露的配置项包括：

- `similarity-threshold`
- `max-edit-distance`
- `max-fix-passes`
- `auto-learn-threshold`
- `keyboard`
- `history.enabled`
- `candidates.enabled`
- `candidates.limit`
- `experimental.long-option-correction.enabled` *（实验性；默认：`false`）*
- `rules.<scope>.enabled`

开启候选选择后，shell 集成会调用 `typo fix --select`。Typo 会把现有最佳修正作为
第一项，再补充有效的命令距离候选，最多展示 `candidates.limit` 条。该限制是最大值；
Typo 不会为了凑满菜单而展示低质量候选。

支持的键盘布局：

- `qwerty`
- `dvorak`
- `colemak`

内置规则作用域：

- `git`、`docker`、`npm`、`yarn`、`kubectl`、`cargo`、`brew`
- `helm`、`terraform`、`python`、`pip`、`go`、`java`、`system`

## 构建与验证

本仓库推荐使用 Makefile 目标进行本地开发：

```bash
make build
make build-all
make install
make test
make coverage
make lint
```

文档相关检查：

- `make markdown-lint`：Markdown 文档检查
- `make codespell-check`：拼写检查

## 相关文档

- CLI 用法请看 [命令参考](commands_CN.md)。
- v1.x 稳定性承诺请看 [稳定性契约](stability_CN.md)。
- 安装说明请看 [快速开始](../getting-started/quick-start_CN.md)。
- 用户视角的修正场景请看 [使用示例](../example/use_CN.md)。
