...
缓存文件可能采用不同处理方式。如果 `subcommands.json` 的格式发生变化，Typo 可以把旧缓存隔离为 `subcommands.json.corrupt-<时间戳>`，然后重新生成，而不是原地迁移。该行为是允许的，因为 `subcommands.json` 只包含动态发现到的命令元数据。

## 配置文件格式

**级别：稳定**

以下 `config.json` 字段名在 v1.x 期间不会被重命名或移除：
...
`typo config` CLI 对顶层数值配置使用带连字符的键名；CLI 写法请看[命令参考](commands_CN.md)。

次版本可能会添加新键。无法识别的键会被忽略，因此由较新 v1.x 版本写入的配置文件
仍然可以在较旧的 v1.x 二进制文件中加载（未知键会被静默跳过）。

次版本也可能引入实验性配置键。实验性配置是显式 opt-in 的，不受上面的稳定字段名承诺约束。

`usage_history.json` 的条目可能包含 `rule_applied: true`，表示该精确
`(from, to)` pair 已经提升为用户规则，不应再继续增长历史计数。
...
- 默认值：`false`
- 行为：仅开启 `--long-option` 级别的实验性 typo 修正

像 `-A`、`-v` 这样的短参数会被明确排除在该实验功能的模糊修正范围之外。
...
- **触发绑定**：`Esc` `Esc` 是默认键绑定，将保持为默认值。用户可以按照故障排除
  指南中的说明重新绑定到其他键。
- **候选选择**：当 `candidates.enabled=true` 时，shell 集成会使用
  `typo fix --select` 显示终端候选菜单。菜单支持数字键、上下键加回车，以及
  `q`/Esc 取消；stdout 仍只输出最终选中的命令。
...
- **Stderr 缓存**：zsh、bash 和 PowerShell 集成会创建临时 `typo-stderr-*` 文件，
  ...
  但内部文件命名可能会更改。
- **Shell 修正上下文**：受支持的 shell 集成可以把临时 `typo-alias-*` 上下文文件传给
  `typo fix --alias-context`。该文件可以包含 alias 类命令缩写和环境变量名。该 flag
  是稳定接口；内部环境变量名和临时文件命名不属于公共 API。

### 可能变更的内容

- Shell 初始化脚本的内部实现（函数名、内部变量）不是公共 API 的一部分，可能在
  任何版本中更改。
- `typo doctor` 文本输出的确切格式是信息性的，可能在次版本中更改。需要程序化诊断时请使用
  `typo doctor --json`。
