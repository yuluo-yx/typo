...
如果执行 `typo fix --select ...` 时只输出最佳修正而没有出现菜单，请先检查
`typo config get candidates.enabled`。当值为 `false` 时，Typo 会向 stderr 写入提示，
并继续让 stdout 只输出最佳修正。可通过 `typo config set candidates.enabled true`
开启候选菜单。
...
依赖本次 `stderr` 报错的修正（包括权限错误产生的 `sudo` 建议）不会写入历史记录或触发自动学习。
该规则同样适用于 `--select` 的单条候选、菜单选择及无可交互终端时的最佳候选回退。
即使后续步骤继续修正了拼写，整条修正仍保留报错上下文标记；独立的普通拼写候选仍按现有配置记录历史。

备选候选保留已知正确的命令和可恢复的 shell 别名，不会仅因拼写相近就替换它们。
菜单将换行、制表符和其他控制字符显示为转义文本；选择后输出的命令仍保留原始内容。
命令首尾的空白不计入历史记录键，因此带首尾空白和不带首尾空白的输入会共用学习计数。
`history.enabled=false` 和 `--no-history` 控制历史写入；已有历史仍可用于纠错。

命令匹配优先保留已知可执行文件的精确名称；内建拼写规则不会覆盖同名可执行文件，显式用户规则仍可覆盖。
模糊匹配按编辑距离、相似度和既有的相邻字符换位例外筛选候选，再按换位、距离、相似度、规则优先级和字典序排序。
没有达标候选时不输出修正，`--debug` 仍会说明被拒绝的近邻。
