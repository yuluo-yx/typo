...
## [2.0.0](https://github.com/yuluo-yx/typo/compare/v1.8.3...v2.0.0) (2026-09-18)
...
* update homebrow ([f5f51d7](https://github.com/yuluo-yx/typo/commit/f5f51d78f4d4eaaf8fa4b09283f9388447c5bb9f))
...
* **brew:** restore the project tap [skip ci] ([dcaf3db](https://github.com/yuluo-yx/typo/commit/dcaf3db9cc3a2ae0d1ef460b0ee563c95b492765))
...
* preserve shell correction targets and arguments ([#237](https://github.com/yuluo-yx/typo/issues/237)) ([86de533](https://github.com/yuluo-yx/typo/commit/86de533e8c161073366233c9d5ee6a552e305f2e))
...
* 修复 `typo fix --select` 丢失候选错误上下文的问题，防止依赖 stderr 的修正写入历史记录或触发自动学习。
* 覆盖单条候选、菜单选择、终端不可用回退、多轮修正和别名场景，保留普通拼写修正的历史记录策略。
* 修复备选候选误改已知正确命令及 shell 别名的问题。
* 修复菜单重绘覆盖上一行，以及命令中的控制字符破坏菜单显示的问题。
* 统一命令首尾空白的历史记录键，避免历史计数分散和自动学习失效。
* 修复 `typo config gen --help` 的退出码。
* 安装器通过最新正式版接口查询版本，避免误选预发布版；Unix 安装器兼容紧凑 JSON，并在下载前拒绝多余位置参数。
* 本地提交钩子的 tidy 和漏洞检查复用当前 Go 工具链与已安装的 govulncheck，避免提交时另行下载 Go。
* 修复模糊匹配中未达标近邻遮挡有效候选、内建规则覆盖已有可执行文件，以及极大编辑距离导致索引溢出的问题。

### 性能

* 精确命令直接查询索引；最佳候选改为单次扫描，规则目标权重在规则变更时更新缓存。
* 编辑距离计算跳过公共前后缀，保持三种键盘布局的既有权重、整数距离和字符换位规则。

发布二进制使用 Go 1.26.6 构建，模块最低 Go 版本保持不变。

无迁移，直接替换二进制；配置格式不变。已有历史记录和用户规则不会自动清理。

10,000 命令词库的匹配基准（同机 5 次中位数）：模糊匹配约 22.0 ms → 2.35 ms，单次分配内存减少约 75%。仅代表进程内匹配函数，详见 [基准说明](https://github.com/yuluo-yx/typo/blob/v1.8.2/benchmarks/matching_CN.md)。

### 构建与依赖

* 合入依赖升级 PR #230、#231、#232：CodeQL init/analyze 配套升级至 4.37.9，Homebrew setup-homebrew 升级至 2026.08.31.1。
* 常规 CI 与 Release CI 固定使用 Go 1.26.6，最低 Go 版本保持 1.26.3。
* 二进制、SHA-256 校验和及 Homebrew formula 由现有 Release CI 生成。

回滚：重新安装 v1.8.1 二进制，保留当前配置和用户规则。
